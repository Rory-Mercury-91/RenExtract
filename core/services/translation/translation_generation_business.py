...
class TranslationGenerationBusiness:
    ...
    def create_language_selector_modular(self, project_path: str, language: str, 
                                    include_language: bool = True, 
                                    include_fontsize: bool = False):
        """
        Fonction modulaire pour créer les sélecteurs selon les besoins.
        Version améliorée avec validation automatique du screen say.
        """
        try:
            game_dir = os.path.join(project_path, "game")
            ...
            if not screens_file:
                return False, "Fichier screens.rpy introuvable dans le dossier game/ (hors traductions)"
            
            log_message("INFO", f"Utilisation de screens.rpy depuis : {os.path.relpath(screens_file, project_path)}", category="renpy_generator_tl")
            ...
            try:
                ...
                return False, f"Impossible de lire screens.rpy : {e}"
            
            # Parser le screen preferences (doit être au niveau 0)
            parsed_screen = self._parse_preferences_screen(screens_content)
            if not parsed_screen:
                return False, "Structure 'screen preferences():' introuvable dans screens.rpy. Le fichier doit contenir un screen preferences() au niveau 0 (tab 0) pour pouvoir être modifié."
            
            # Vérifier ce qui existe déjà 
            language_exists = self._language_already_exists(parsed_screen['content'], language) if include_language else False
            fontsize_exists = self._check_if_fontsize_exists(parsed_screen['content']) if include_fontsize else False
            
            # Déterminer ce qu'il faut ajouter
            add_language = include_language and not language_exists
            add_fontsize = include_fontsize and not fontsize_exists
            
            if not add_language and not add_fontsize:
                existing_components = []
                if language_exists:
                    existing_components.append("sélecteur de langue")
                if fontsize_exists:
                    existing_components.append("contrôle de taille")
                return True, f"{', '.join(existing_components)} déjà présent(s) dans le menu"
            
            # NOUVEAU : Validation du screen say si contrôle de taille demandé
            use_precise_control = False
            fontsize_method = "global"
            
            if add_fontsize:
                is_valid, validation_msg = self._validate_say_screen_structure(project_path)
                if is_valid:
                    use_precise_control = True
                    fontsize_method = "précis (dialogue uniquement)"
                    log_message("INFO", f"Screen say standard détecté - utilisation contrôle précis", 
                            category="renpy_generator_tl")
                else:
                    log_message("INFO", f"Screen say non standard ({validation_msg}) - fallback contrôle global", 
                            category="renpy_generator_tl")
            ...
            # Injection modulaire
            modified_screen = self._inject_modular_selectors(
                parsed_screen, language, add_language, add_fontsize, use_precise_control
            )
            
            # Nom de fichier selon le contenu
            if add_language and add_fontsize:
                filename = "99_Z_LangSelect_FontSize.rpy"
            elif add_language:
                filename = "99_Z_LangSelect.rpy"
            else:
                filename = "99_Z_FontSize.rpy"
            
            selector_file = tl_lang_dir / filename
            self._write_modular_screen_with_fontsize(
                selector_file, modified_screen, language, 
                add_language, add_fontsize, use_precise_control
            )
            
            # Message de résultat
            components = []
            if add_language:
                components.append("sélecteur de langue")
            if add_fontsize:
                components.append(f"contrôle de taille ({fontsize_method})")
            
            message = f"{' + '.join(components)} intégré(s) pour '{language}'"
            log_message("INFO", f"Sélecteurs modulaires créés : {filename}", category="renpy_generator_tl")
            return True, message
            
        except Exception as e:
            log_message("ERREUR", f"Erreur création sélecteurs modulaires : {e}", category="renpy_generator_tl")
            return False, f"Erreur lors de la création : {e}"

    def _inject_modular_selectors(self, parsed_screen: dict, language: str, 
                                add_language: bool, add_fontsize: bool, 
                                use_precise_control: bool = False) -> dict:  # ← Ajouter ce paramètre
        """
        Injection modulaire selon les composants demandés
        """
        try:
            lines = parsed_screen['content'].copy()
            ...
            if add_language and add_fontsize:
                # Les deux ensemble
                if use_precise_control:
                    vbox_content = self._generate_language_and_fontsize_precise(language, lines)
                else:
                    vbox_content = self._generate_language_and_fontsize_vbox(language, lines)
            elif add_language:
                # Seulement langue
                vbox_content = self._generate_language_vbox_simple(language, lines)
            elif add_fontsize:
                # Seulement taille
                if use_precise_control:
                    vbox_content = self._generate_fontsize_bar_precise(lines)
                else:
                    vbox_content = self._generate_fontsize_bar_only(lines)
            
            if vbox_content and injection_point is not None:
                lines[injection_point:injection_point] = vbox_content
            elif vbox_content:
                insert_pos = len(lines) // 3
                lines[insert_pos:insert_pos] = vbox_content
            
            parsed_screen['content'] = lines
            return parsed_screen
            
        except Exception as e:
            log_message("ERREUR", f"Erreur injection modulaire : {e}", category="renpy_generator_tl")
            return parsed_screen

    def _generate_language_and_fontsize_precise(self, language: str, existing_lines: list) -> list:
        """
        Génère les deux vbox ensemble (langue + taille précise)
        """
        try:
            vbox_indent = "        "
            ...
            for i, line in enumerate(existing_lines):
                if 'label _("Skip")' in line:
                    for j in range(i, -1, -1):
                        if existing_lines[j].strip().startswith("vbox:"):
                            vbox_indent = existing_lines[j][:len(existing_lines[j]) - len(existing_lines[j].lstrip())]
                            break
                    break
            ...
            combined_vbox = [
                "",
                f"{vbox_indent}vbox:",
                f"{vbox_indent}    style_prefix \"radio\"",
                f"{vbox_indent}    label _(\"Language\")",
                f"{vbox_indent}    textbutton _(\"English\") action Language(None)",
                f"{vbox_indent}    textbutton _(\"{display_name}\") action Language(\"{language}\")",
                "",
                f"{vbox_indent}vbox:",
                f"{vbox_indent}    style_prefix \"slider\"",
                f"{vbox_indent}    label _(\"Dialogue Size\")",
                f"{vbox_indent}    text \"Taille: [persistent.renextract_dialogue_size]/75\" size 14",
                f"{vbox_indent}    bar:",
                f"{vbox_indent}        value FieldValue(persistent, 'renextract_dialogue_size', range=75, max_is_zero=False, style='slider', offset=0, step=1)",
                "",
                f"{vbox_indent}    # Bouton de réinitialisation",
                f"{vbox_indent}    if persistent.renextract_dialogue_size != 34:",
                f"{vbox_indent}        textbutton _(\"Reset dialogue settings\"):",
                f"{vbox_indent}            action [",
                f"{vbox_indent}                SetField(persistent, 'renextract_dialogue_size', 34),",
                f"{vbox_indent}                SetField(persistent, 'renextract_name_size_offset', 10)",
                f"{vbox_indent}            ]",
                ""
            ]

            return combined_vbox

        except Exception as e:
            log_message("ERREUR", f"Erreur génération vbox combinée précise : {e}", category="renpy_generator_tl")
            return []

    def _write_fontsize_only_screen_to_file(self, file_path: Path, parsed_screen: dict, language: str):
        """
        Écrit le screen avec seulement le contrôle de taille pour dialogues
        """
        try:
            content = (
                "# Auto-generated dialogue font size control\n"
                "# Contrôle de taille de police pour dialogues uniquement\n"
                f"# Langue cible : {language}\n\n"
                
                "# Initialisation du système de taille de police dialogue\n"
                "init python:\n"
                "    # Variable persistante pour la taille des dialogues\n"
                "    if not hasattr(persistent, 'dialogue_text_size'):\n"
                "        persistent.dialogue_text_size = 1.0  # 100% par défaut\n"
                "    \n"
                "    # Style personnalisé pour les dialogues\n"
                "    def update_dialogue_style():\n"
                "        # Mettre à jour la taille du texte de dialogue\n"
                "        style.say_dialogue.size = int(gui.text_size * persistent.dialogue_text_size)\n"
                "        style.say_thought.size = int(gui.text_size * persistent.dialogue_text_size)\n"
                "    \n"
                "    # Appliquer au démarrage\n"
                "    config.start_callbacks.append(update_dialogue_style)\n"
                "    \n"
                "    # Callback pour mettre à jour en temps réel\n"
                "    def dialogue_size_changed(value):\n"
                "        persistent.dialogue_text_size = value\n"
                "        update_dialogue_style()\n"
            ...
            if lang_norm == "french":
                content += (
                    ...
                    '\n'
                    '    old "Dialogue Text Size"\n'
                    '    new "Taille du texte de dialogue"\n'
                )
            
            file_path.write_text(content, encoding="utf-8", newline="\n")
            log_message("INFO", f"Screen taille dialogue sauvegardé : {file_path.name}", category="renpy_generator_tl")
            
        except Exception as e:
            log_message("ERREUR", f"Erreur écriture screen taille dialogue : {e}", category="renpy_generator_tl")
            raise

    def _write_modular_screen_with_fontsize(self, file_path: Path, parsed_screen: dict, 
                                            language: str, include_language: bool, 
                                            include_fontsize: bool, use_precise_control: bool):
        """
        Écrit le screen avec le système de contrôle de taille approprié.
        """
        try:
            # Entête selon les composants
            components = []
            if include_language:
                components.append("sélecteur de langue")
            if include_fontsize:
                method = "contrôle précis dialogue" if use_precise_control else "contrôle global texte"
                components.append(method)
            
            content = (
                f"# Auto-generated {' + '.join(components)}\n"
                "# Intégration intelligente dans le screen preferences\n"
                f"# Langue cible : {language}\n\n"
            )
            
            # Ajouter le système de contrôle de taille si nécessaire
            if include_fontsize:
                if use_precise_control:
                    content += self._generate_precise_dialogue_size_control(language)
                    content += "\n\n"
                else:
                    content += self._generate_global_fontsize_fallback()
                    content += "\n"
            
            # Ajouter le screen preferences modifié
            screen_lines = parsed_screen.get("content", [])
            content += "\n".join(screen_lines) if isinstance(screen_lines, list) else str(screen_lines)
            
            # Traductions selon les composants
            lang_norm = str(language).strip().lower()
            language_translation = self.get_language_word_translation(lang_norm)
            
            if language_translation and (include_language or include_fontsize):
                content += f'\n\ntranslate {lang_norm} strings:\n\n'
                
                if include_language:
                    content += (
                        '    old "Language"\n'
                        f'    new "{language_translation}"\n'
                        '\n'
                    )
                ...
                if include_fontsize:
                    if use_precise_control:
                        content += (
                            '    old "Dialogue Size"\n'
                            '    new "Taille du dialogue"\n'
                            '\n'
                            '    old "Reset dialogue settings"\n'
                            '    new "Réinitialiser les paramètres du dialogue"\n'
                        )
                    else:
                        content += (
                            '    old "Text Size"\n'
            ...
            file_path.write_text(content, encoding="utf-8", newline="\n")
            log_message("INFO", f"Screen modulaire sauvegardé : {file_path.name}", category="renpy_generator_tl")
            
        except Exception as e:
            log_message("ERREUR", f"Erreur écriture screen modulaire : {e}", category="renpy_generator_tl")
            raise

    def _generate_precise_dialogue_size_control(self, language: str) -> str:
        """
        Génère le code pour un contrôle précis de la taille du dialogue.
        Adapté du système RenExtract, renommé pour RenExtract.
        """
        return '''## RenExtract - Contrôle précis de la taille du dialogue
## Système adapté pour screen say standard

# Valeurs par défaut (définies AVANT init python)
define renextract_dialogue_size_default = 34
define renextract_name_size_offset_default = 10

# Initialisation des variables persistantes
default persistent.renextract_dialogue_size = 34
default persistent.renextract_name_size_offset = 10

init python:
    # Sécurité : s'assurer que les valeurs existent
    if persistent.renextract_dialogue_size is None:
        persistent.renextract_dialogue_size = 34
    if persistent.renextract_name_size_offset is None:
        persistent.renextract_name_size_offset = 10

# Redéfinition du screen say avec contrôle de taille
screen say(who, what):
    style_prefix "say"
    ...
    window:
        ...
        $ name_size = persistent.renextract_dialogue_size + persistent.renextract_name_size_offset
        
        if who is not None:
            window:
                id "namebox"
                style "namebox"
                text who id "who" size name_size
        
        text what id "what" size persistent.renextract_dialogue_size
    
    # Image latérale si présente
    if not renpy.variant("small"):
        add SideImage() xalign 0.0 yalign 1.0

'''

    def _generate_global_fontsize_fallback(self) -> str:
        """
        Système de fallback utilisant la préférence globale de Ren'Py.
        """
        return '''## RenExtract - Contrôle global de la taille du texte
## Fallback pour projets avec screen say personnalisé

# Note: Ce contrôle affecte tous les textes du jeu, pas seulement les dialogues
# car le screen say de ce projet utilise une structure personnalisée.

'''

    def _generate_fontsize_bar_precise(self, existing_lines: list) -> list:
        """
        Génère la vbox pour le contrôle précis de taille du dialogue.
        """
        try:
            vbox_indent = "        "
            ...
            return fontsize_vbox

        except Exception as e:
            log_message("ERREUR", f"Erreur génération barre dialogue précise : {e}", category="renpy_generator_tl")
            return []

    def _generate_language_vbox_simple(self, language: str, existing_lines: list) -> list:
        """
        Génère seulement la vbox de sélection de langue (sans taille).
        """
        try:
            vbox_indent = "        "
            ...
            return language_vbox

        except Exception as e:
            log_message("ERREUR", f"Erreur génération vbox langue simple : {e}", category="renpy_generator_tl")
            return []

    def _generate_language_and_fontsize_vbox(self, language: str, existing_lines: list) -> list:
        """
        Génère les deux vbox ensemble (langue + taille)
        """
        try:
            vbox_indent = "        "
            ...
            combined_vbox = [
                ...
                f"{vbox_indent}vbox:",
                f"{vbox_indent}    style_prefix \"slider\"",
                f"{vbox_indent}    label _(\"Text Size\")",
                f"{vbox_indent}    bar:",
                f"{vbox_indent}        value Preference(\"font size\")",
                f"{vbox_indent}        range 0.5",
                f"{vbox_indent}        style \"slider\"",
                f"{vbox_indent}    text \"[preferences.font_size:.0%]\" xalign 0.5 size 14",
                ""
            ]
            ...
            log_message("ERREUR", f"Erreur génération vbox combinée : {e}", category="renpy_generator_tl")
            return []

    def _find_injection_point(self, lines: list) -> int:
        """
        Trouve le meilleur point d'injection dans le screen.
        """
        injection_point = None
        
        # Méthode 1: Après la vbox Skip
        for i, line in enumerate(lines):
            if 'label _("Skip")' in line:
                vbox_indent = len(line) - len(line.lstrip())
                for j in range(i + 1, len(lines)):
                    next_line = lines[j]
                    if next_line.strip() == '':
                        continue
                    next_indent = len(next_line) - len(next_line.lstrip())
                    if (next_indent <= vbox_indent and 
                        (next_line.strip().startswith('vbox:') or next_line.strip().startswith('##'))):
                        injection_point = j
                        break
                if injection_point:
                    break
        
        # Méthode 2: Après le premier hbox
        if injection_point is None:
            for i, line in enumerate(lines):
                if 'hbox:' in line and i+1 < len(lines) and 'box_wrap True' in lines[i+1]:
                    hbox_indent = len(line) - len(line.lstrip())
                    for j in range(i + 1, len(lines)):
                        next_line = lines[j]
                        if next_line.strip() == '':
                            continue
                        next_indent = len(next_line) - len(next_line.lstrip())
                        if next_indent <= hbox_indent and next_line.strip() and not next_line.strip().startswith('vbox:'):
                            injection_point = j
                            break
                    break
    ...
    def _generate_fontsize_bar_only(self, existing_lines: list) -> list:
        """
        Génère seulement la barre de taille de police (sans langue).
        """
        try:
            vbox_indent = "        "
            ...
            log_message("ERREUR", f"Erreur génération barre police seule : {e}", category="renpy_generator_tl")
            return []
    ...
    def create_individual_font_system_file(self, project_path: str, language: str, font_options: dict):
        ...
        try:
            ...
            font_rpy = tl_lang_dir / "0-font-system.rpy"
